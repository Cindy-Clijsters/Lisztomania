<?php
return [
    
    'field.name' => 'Naam',
    'field.status' => 'Status',
    
    'status.makeChoice' => '- Kies een status -',
    'status.active' => 'Actief',
    'status.inactive' => 'Inactief',
    
    'action.view' => 'Een label bekijken',
    'action.add' => 'Een label toevoegen',
    'action.update' => 'Een label wijzigen',
    'action.delete' => 'Een label verwijderen',
    
    'list.noFound' => 'Er zijn geen labels gevonden.',
    'list.showEntries' => 'Toon %start% tot %end% van %total% labels',
    
    'error.noLabelWithSlug' => 'Er is geen label gevonden met het slug \'%slug%\'',
    'error.deleteFailed' => 'Het label "%name%" kan niet verwijderd worden.',
    'error.labelLinkedToAlbums' => 'Het is gekoppeld aan één of meerdere albums.',
    
    'msg.addedSuccessfully' => 'Het label "%name%" is met succes toegevoegd.',
    'msg.changedSuccessfully' => 'Het label "%name%" is met succes gewijzigd.',
    'msg.deletedSuccessfully' => 'Het label "%name%" is met succes verwijderd.',
    
];